...
def app():
    st.markdown(
        """
        <style>
        /* Force text inside text_input fields to be white */
        div[data-baseweb="input"] > div > input {
            color: white !important;
        }

        /* Force text inside selectbox to be white */
    ...
    while True:
        ...
        with mean_placeholder.container():
            ...
            with plot1:
                ...
                fig = px.pie(readmit_counts, names='Readmission Status', values='Count', 
                           title='Readmission Status Distribution',
                           color_discrete_sequence=px.colors.sequential.Greens[::3])
                # st.write(fig, key=f"pie_chart_{time.time()}")
                st.plotly_chart(fig, key=f"pie_chart_{time.time()}")
            with plot2:
                avg_time = data.groupby('medical_specialty')['time_in_hospital'].mean().reset_index()
                fig = px.bar(avg_time, x='medical_specialty', y='time_in_hospital', 
                           title='Average Time in Hospital by Medical Speciality',
                           color_discrete_sequence=['green'])
                fig.update_layout(xaxis_tickangle=-45)
                # st.write(fig, key=f"bar_chart_{time.time()}")
                st.plotly_chart(fig, key=f"bar_chart_{time.time()}")
        ...
        with individual_placeholder.container():
            ...
            fig = px.line(df_preprocessed, x='timestamp', y='readmission_probability', 
                          title='Readmission Probability Over Time', 
                          labels={'readmission_probability': 'Probability'},
                          color_discrete_sequence=['green'])
            fig.update_traces(fill='tozeroy', fillcolor='rgba(0, 255, 0, 0.2)')
            st.plotly_chart(fig, key=f"readmission_prob_chart_{time.time()}")
        ...
        with individual_placeholder_2.container():
            ...
            with fig_col1:
                fig = px.line(
                    ...
                    title='Number of Lab Procedures Over Time',
                    labels={'num_lab_procedures': 'Number of Lab Procedures'},
                    color_discrete_sequence=['green']
                )
                fig.update_traces(fill='tozeroy', fillcolor='rgba(0, 255, 0, 0.2)')
                st.plotly_chart(fig, key=f"lab_procedures_chart_{time.time()}")
            ...
            with fig_col2:
                fig = px.line(
                    ...
                    x='timestamp',
                    y='num_medications',
                    title='Number of Medications Over Time',
                    labels={'num_medications': 'Number of Medications'},
                    color_discrete_sequence=['green']
                )
                fig.update_traces(fill='tozeroy', fillcolor='rgba(0, 255, 0, 0.2)')
                st.plotly_chart(fig, key=f"medications_chart_{time.time()}")
            ...
            with fig_col3:
                fig = px.line(
                    ...
                    x='timestamp',
                    y='time_in_hospital',
                    title='Time in Hospital Over Time',
                    labels={'time_in_hospital': 'Time in Hospital (Days)'},
                    color_discrete_sequence=['green']
                )
                fig.update_traces(fill='tozeroy', fillcolor='rgba(0, 255, 0, 0.2)')
                st.plotly_chart(fig, key=f"hospital_time_chart_{time.time()}")

            with fig_col4:
                # Convert HbA1c_result to numeric for plotting
                ...
                fig = px.line(
                    df_preprocessed,
                    x='timestamp',
                    y='HbA1c_result_numeric',
                    title='HbA1c Result Over Time',
                    labels={'HbA1c_result_numeric': 'HbA1c Result'},
                    color_discrete_sequence=['green']
                )
                fig.update_traces(fill='tozeroy', fillcolor='rgba(0, 255, 0, 0.2)')
                st.plotly_chart(fig, key=f"hba1c_chart_{time.time()}")
